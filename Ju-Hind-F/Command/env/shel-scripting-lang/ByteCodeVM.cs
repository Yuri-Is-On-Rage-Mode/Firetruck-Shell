...
namespace firetruck.Command.env.shel_scripting_lang.ByteCode
{
    internal class ByteCodeVM
    {
        ...
        public class VirtualMachine
        {
            ...
            public static void StartRunningTheCodes(List<string> code)
            {
                ...
                {
                    firetruckErrorList.New("UserDefined: Code: Is Empty!");
                    firetruckErrorList.ListThem();
                }
            }
        ...
        public class Process
        {
            public class ByteCode
            {
                ...
                public static void Please(List<string> codes)
                {
                    ...
                    if (codeLen >= MaxLen)
                    {
                        firetruckErrorList.New($"VM: Message: Your code is bigger than expected!\n\tYour Code Length: {codeLen}\n\tMax Code Process Length: {MaxLen}");
                        firetruckErrorList.ListThem();
                    }
                    else
                    {
                        firetruckWarningList.New("ByteCode Virtual Machine: May cause some errors!");
                        firetruckWarningList.ListThem();
                        firetruckWarningList.CacheClean();
